...
class OnlineBankStatementProviderQonto(models.Model):
    ...
    def _qonto_get_slug(self):
        ...
        response = requests.get(url, headers=self._qonto_header())
    ...
    def _qonto_get_transactions(self, url, params):
        response = requests.get(
            url, params=params, headers=self._qonto_header()
        )
        if response.status_code == 200:
    ...
    def _qonto_prepare_statement_line(
        ...
        if (
            transaction["currency"] not in currencies_code2id
            or transaction["local_currency"] not in currencies_code2id
        ):
            raise UserError(
                _("Currency %s used in transaction ID %s doesn't exist " "in Odoo.")
                % (transaction["currency"], transaction["transaction_id"])
            )
